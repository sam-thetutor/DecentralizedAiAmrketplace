...
actor class Backend() = this {
    ...
    stable var adminPrincipal : Principal = Principal.fromText("7ng2f-cxxjz-gd2mx-7cxja-dfz4s-tzcvp-kjrsx-4ncw2-vmkps-bfxqw-bae");
    ...
    //store the task to agent mapping
    private stable var taskToAgentArray : [(Nat, Principal)] = [];
    private var taskToAgentMap = HashMap.fromIter<Nat, Principal>(
        Iter.fromArray(taskToAgentArray),
        Iter.size(Iter.fromArray(taskToAgentArray)),
        Nat.equal,
        Hash.hash,
    );
    type TaskAgentMapping = {
        taskId: Nat;
        agentId: Principal;
    };

        //private var taskToAgentMap = HashMap.HashMap<Nat, Principal>(0, Nat.equal, Hash.hash);
    ...
    //register the agent
    public shared({caller}) func registerAgent(agentAddress:Principal,metadata: AgentDescription,price:Nat):async Result.Result<Text,Text>{
        assert(Owner == caller);
        //check if the agent already exists
        switch(metadataHashMap.get(caller)){
            case(null){
                 metadataHashMap.put(agentAddress, {metadata = metadata; Price = price; reputation = 0});
    ...
    //manually update your balance
    public shared({caller}) func updateBalance(balance:Nat):async Result.Result<Text,Text>{
        // assert(Owner == caller);
        balanceHashMap.put(caller, balance);
        return #ok("Balance updated");
    };


    //update the agent
    public shared({caller}) func updateAgent(metadata:AgentDescription):async Result.Result<Text,Text>{
        assert(Owner == caller);
        //check if the agent exists
        switch(metadataHashMap.get(caller)){
            case(null){
    ...
    // Add these functions
    public shared({caller}) func assignTaskToAgent(taskId: Nat, agentId: Principal) : async Result.Result<Text, Text> {
        // assert(adminPrincipal == caller);
        taskToAgentMap.put(taskId, agentId);
        return #ok("Task assigned successfully");
    };

    public query func getTaskAgent(taskId: Nat) : async ?Principal {
        return taskToAgentMap.get(taskId);
    };
